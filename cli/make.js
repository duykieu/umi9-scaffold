...
const makeController = (args) => {
    ...
    const templatePath = path.join(__dirname, "..", "cli/template/controller.txt");
    let fileContent = fs.readFileSync(templatePath, "utf-8");

    fileContent = fileContent.replace(/%MODEL_NAME%/g, modelName);
    fileContent = fileContent.replace(/%PLURAL_NAME%/g, pluralize(_.camelCase(modelName)));
    ...
    const routerTemplatePath = path.join(__dirname, "..", "cli/template/router.txt");
...
//umi-import-do-not-delete`
    ...
    fs.writeFile(path.join(__dirname, "..", "routers.js"), mainRouterContent, (err) => {
        if (err) return console.log(`${err}`.red);
...
module.exports = ({ command, args }) => {
    ...
    if (command === "make:superuser") {
        ...
        User.create({ ...user, passwordConfirm: user.password, userGroup: "super_adm" }, (err) => {
            if (err) return console.log(`${err}`.red);
    ...
    if (command === "make:model") {
        ...
        fileContent = fileContent.replace(/%MODEL_NAME%/g, modelName);
        fileContent = fileContent.replace(/%MODEL_FIELDS%/g, modelConfigurationString);
        fileContent = fileContent.replace(/%TABLE_NAME%/g, pluralize(_.snakeCase(modelName)));
